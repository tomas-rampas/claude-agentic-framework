...
This is enforced by a real Claude Code `Stop` hook: `hooks/stop-peer-review-gate.ps1`, registered in `settings.json` under `hooks.Stop` (distributed via `settings.template.json`; installed with `scripts/install.ps1`). It blocks ending a session when the current feature branch has committed work ahead of its base and `peer-review-critic` has not run that session — a companion `PostToolUse` hook (`hooks/record-subagent-run.ps1`) records peer-review-critic runs per session. The gate is loop-safe (respects `stop_hook_active`), fail-open on any error, and fires at most once per session.
